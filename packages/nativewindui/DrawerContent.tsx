...
type DrawerContentRootProps = React.ComponentProps<typeof View> & {
...
function DrawerContentSection({ className, ...props }: React.ComponentProps<typeof View>) {
  return <View className={cn('gap-1 px-2', className)} {...props} />;
}

function DrawerContentSectionItem({
  label,
  icon,
  onPress,
  isActive,
