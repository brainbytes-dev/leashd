...
import { z } from "zod";
import { MoneyUnit, Rail, type PaymentRequest } from "@repo/leash-core";
...
function toRequest(args: PayArgs, agentId: string): PaymentRequest {
  return {
    agentId,
    rail: args.rail,
    amount: args.amount,
...
export function createMcpServer(deps: {
  ...
  server.registerTool(
    ...
    {
      description:
        "Settle a payment on the configured rail, gated by the local Leash policy " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Dry-run: evaluate a hypothetical payment against the current policy WITHOUT " +
