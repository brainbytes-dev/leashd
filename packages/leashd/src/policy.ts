 ...
 */
export function loadAgentPolicy(
  store: Store,
  config: LeashConfig,
  agentId: string
  ...
  if (!config.controlPlanePubKey) return undefined;
 ...
 */
export function applySignedPolicy(
  store: Store,
  config: LeashConfig,
  agentId: string,
  specJson: string,
  ...
  if (!config.controlPlanePubKey) return false;
  ...
  if (existing && existing.version > spec.version) return false;
